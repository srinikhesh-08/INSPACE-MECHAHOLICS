...
- Memory Architecture — Function-Oriented Design
...
- Function-Oriented, Not Slot-Based
- The CDH memory architecture is organised by function rather than predefined storage "slots." Three distinct resources serve complementary roles spanning firmware storage, runtime operation, and mission data persistence.
- STM32F405 Internal Flash
- Non-volatile (onboard). Flight software storage, configuration parameters, configurable FSM thresholds, system initialisation data
- STM32F405 Internal SRAM
- Volatile (onboard). Sensor acquisition buffers, telemetry packet assembly, temporary mission data processing, communication buffers
- microSD Card (FAT32)
- Non-volatile (removable). Telemetry logging, mission-state logging, recovery information storage, post-flight data retrieval
- A subset of microSD data — mission elapsed time, packet counter, and FSM state — is periodically persisted to support reset recovery (see Mission Continuity slide).
- Compliance Statement
- Memory Requirements — SATISFIED
- Satisfied through the combination of internal Flash (firmware + config), internal SRAM (runtime operation), and FAT32 microSD (mission data storage and persistence) — no fixed number of discrete storage slots required

- Why Function-Oriented?
- Defining memory by role rather than by a fixed slot count reflects how embedded flight software actually allocates resources: firmware and configuration are compiled into Flash at build time, runtime data structures occupy SRAM during operation, and mission records accumulate on removable storage independent of the processor's internal memory map.
- This framing avoids an arbitrary "number of slots" specification that would not map cleanly onto the STM32F405's actual memory architecture, while still demonstrating that every functional memory requirement — firmware, configuration, runtime buffers, and mission data — is met by a specific, identified resource.
...
- Primary — Real-Time Telemetry
- SX1276 LoRa downlink to GCS at 1 Hz — live mission monitoring
- Secondary — Onboard Data Preservation
- Simultaneous logging of the same packet to FAT32 microSD, independent of RF link status
- Key Outcome
...
- Team LOGO/NAME
- IN-SPACe Model Rocketry/CAN-7USAT India Student Competition 2026:: Team Name
- This separates what is implemented now (dual-path data protection, reserved opcode) from what is intentionally deferred (specific channel frequencies/parameters).
